...
internal class RestTest {
    ...
    @Test fun `Media type is calculated properly`() {
        ...
        assertEquals(APPLICATION_JSON, HttpServerRequest().mediaType())
        assertEquals(APPLICATION_JSON, HttpClientResponse().mediaType())
        HttpServerRequest(contentType = ContentType(APPLICATION_YAML)).apply {
            assertEquals(APPLICATION_YAML, mediaType())
        }
        HttpClientResponse(contentType = ContentType(APPLICATION_YAML)).apply {
            assertEquals(APPLICATION_YAML, mediaType())
        }
    }
